...
export const Route = createFileRoute("/authority/live-reports")({
  ...
  head: () => ({
    meta: [
      { title: "Live Reports | Vehicle Authority" },
...
function LiveReportsPage() {
  ...
  const fetchScans = async () => {
    ...
    try {
      ...
      console.log('🔑 Token:', state.access_token.substring(0, 20) + '...');
  ...
  return (
    <AuthorityShell title="Live Reports" subtitle="Real-time QR Scan Reports">
      {/* Controls */}
          ...
          <div className="flex items-center justify-between">
            ...
            {error.includes("session has expired") || error.includes("Not authenticated") ? (
              <button
                ...
                className="border-4 border-destructive bg-destructive px-4 py-2 font-bold uppercase text-white hover:bg-destructive/90"
              >
                Login Again
              </button>
            ) : (
