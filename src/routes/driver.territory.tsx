...
import { CheckCircle2, Check, X, Building2, MapPin, Truck, AlertCircle, RefreshCw, Navigation, Maximize2, Minimize2, Compass } from "lucide-react";
...
function DriverTerritoryPage() {
  ...
  return (
    <DriverShell
      title={`Territory Map: ${kmlData?.driverZoneName || 'Zone A'}`}
      subtitle={`Assigned Vehicle: ${licensePlate || 'MH-15'} • Driver: ${driverName}`}
      flush
    >
      <div className="p-6 space-y-6 max-w-[1700px] mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          {/* LEFT 8-COLS: Full Map View & Controls */}
            ...
            <div className="relative rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl overflow-hidden flex flex-col h-full">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-sm font-black uppercase text-white tracking-wider">
                      {kmlData?.driverZoneName || 'Zone A Territory'} Interactive GPS View
                    </h3>
              ...
              {optimizedRoute && routeInfo && !isAllDone && (
                <div className="bg-gradient-to-r from-orange-600 to-amber-600 px-5 py-3 text-white flex items-center justify-between shadow-md">
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-black/20 text-white font-black">
              ...
              {isAllDone && (
                <div className="bg-emerald-600 px-5 py-3 text-white flex items-center justify-between shadow-md animate-pulse">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="text-xs font-black uppercase tracking-wider block">All Pickups Completed!</span>
                      <span className="text-xs font-semibold opacity-95">Please follow the route back to Central Depot</span>
                    </div>
                  </div>
              ...
              <div className={`${isMapExpanded ? "h-[calc(100vh-120px)]" : "h-[62vh] min-h-[480px]"} w-full relative bg-slate-950`}>
                {mounted ? (
                  <Suspense fallback={
                    <div className="flex h-full w-full items-center justify-center bg-slate-950 text-slate-400">
                      ...
                      <span className="font-bold text-base">Loading high-resolution map engine…</span>
                    </div>
                  }>
                    <RouteMap
                      kmlData={kmlData}
                ...
                <div className="absolute top-4 right-4 z-[400] rounded-xl border border-slate-700 bg-slate-900/95 p-3.5 shadow-2xl backdrop-blur-md text-xs font-bold space-y-2 min-w-[200px]">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    ...
                    <span className="font-black text-orange-400">Zone: {kmlData?.driverZoneName || 'Zone A'}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-300">Progress:</span>
                    <span className="font-black text-emerald-400">{collectedCount} / {bins.length} Picked ({progressPct}%)</span>
                  </div>
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl space-y-4">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4">
                <div>
                  <h3 className="text-xs font-black uppercase text-slate-300 tracking-wider">Territory Collection Status</h3>
                  <div className="text-3xl font-black text-white mt-1">
                    {collectedCount} <span className="text-base font-bold text-slate-400">/ {bins.length} Bins</span>
                  </div>
                </div>
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-orange-600/10 border border-orange-500/30 font-black text-xl text-orange-400">
                  {progressPct}%
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-xs font-bold text-slate-300">
                <span>Pending: {bins.length - collectedCount}</span>
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-xl flex-1 flex flex-col">
              <div className="flex items-center justify-between border-b border-slate-800 pb-4 mb-4">
                <div>
                  <h3 className="text-base font-black text-white">Dustbin Collection Checklist</h3>
                  <p className="text-xs font-semibold text-slate-300 mt-0.5">Click YES to mark collected, NO to reset</p>
                </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        ...
                        {/* YES / NO Toggle Controls with Per-Bin Spinner */}
